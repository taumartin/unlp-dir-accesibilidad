...
const tipoEventoRepository = require("../repositories/tipo-evento").getInstance();
...
module.exports.create = asyncHandler(async function (req, res) {
    ...
    const tipoEvento = await tipoEventoRepository.createTipoEvento(validated.nombre);
...
module.exports.listAll = asyncHandler(async function (req, res) {
    ...
    const response = await tipoEventoRepository.listTiposEventos(parseInt(page) || 1, parseInt(pageSize) || 10,
        search || "", orderBy || "id", orderDirection || "asc",);
    apiResponse.success(res, response);
});

module.exports.findById = asyncHandler(async function (req, res) {
    const tipoEvento = await tipoEventoRepository.findById(req.params.id);
    if (tipoEvento === null) {
...
module.exports.update = asyncHandler(async function (req, res) {
    ...
    const result = await tipoEventoRepository.update(req.params.id, updated);
